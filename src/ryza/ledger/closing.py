    ...
    *,
    ...
    if bt == "fund":
        for iid in _util.held_instruments(conn, book_id):
            ...
            # 破れは**評価替えの経路を一切参照せずに**検出できる。分離前は同じ式が書けな
            # かった(原価と評価調整が同居し、差し引きに推定が要った)。
            #
            # **この恒等式が覆うのは原価勘定側だけである**(独立審査 新-19。設計文書 11
            # §5.2-1 と同じ限界)。評価調整勘定を直接叩く偽装 —— 締めジョブ名を騙った
            # `Dr securities_mtm / Cr capital` —— は**ここでは検出できない**(審査実測:
            # NAV 13,000,000→10,000,000、`unexplained_residue` は空)。その経路の防御は
            # `post_mark_to_market` の posted_by 検証と 0034 の DB トリガであって、
            # 恒等式ではない。恒等式が実際に名指しするのは、逆仕訳のオペミス(買いだけを
            # 取り消して売りを残す —— 新-15 の実測ケース)と、数量を再生できない証憑で
