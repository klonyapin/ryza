#!/usr/bin/env bash
#
# deploy-a13.sh — A-13 監査(規則⇔実装トレーサビリティ)を GCE VM の週次 systemd timer で常駐設置する。
#
# 冪等: 何度再実行してもよい。git 導入・監査用 clone 作成・ランナー/unit 設置だけを行う。
#
# 構成判断(定款第5〜6条・独立役員審査 2 回の指摘を反映):
#   - Cloud Run 版 ops-weekly は checkout も VM 内 DB も持たないため A-13 は恒久スキップになる。
#     A-13 は VM(ryza-bot)上の独立 timer で走らせ、結果 embed は press.outbox 経由で #運営 へ届く
...
#     rsync でデプロイされた a13 実装の改変が監査を無害化する経路を断つ(独立役員懸念2)。
...
#
# 使い方: ./ops/deploy-a13.sh   (PROJECT/ZONE/VM/ONCALENDAR は env で上書き可)
set -euo pipefail
...
A13_ONCALENDAR="${A13_ONCALENDAR:-Mon *-*-* 01:40:00 UTC}"
...
cat > /opt/ryza-a13-run.sh <<'RUNNER'
#!/usr/bin/env bash
set -euo pipefail
...
exec /opt/ryza/.venv/bin/python -m ryza.audit.a13 --repo "\${AUDIT}" --always-report
RUNNER
chmod 700 /opt/ryza-a13-run.sh

# 失敗通知(インフラ層の失敗も #運営 へ。これ自体の失敗は journal のみが限界)
cat > /opt/ryza-a13-fail.sh <<'FAILSH'
#!/usr/bin/env bash
...
with connect() as conn:
    r = start_run("audit.a13.failure", conn=conn)
    outbox.enqueue(
        conn, "ops",
        {"title": "A-13 実行失敗(インフラ層)",
         "description": "systemd ryza-a13.service が失敗しました。clone/fetch/DB/コード層の障害の可能性。"
                        "journalctl -u ryza-a13 を確認してください。",
         "color": 15158332},
...
PY
FAILSH
chmod 700 /opt/ryza-a13-fail.sh

# systemd units + timer
cat > /etc/systemd/system/ryza-a13-fail.service <<'FAILUNIT'
[Unit]
Description=Ryza A-13 失敗通知

[Service]
Type=oneshot
Environment=RYZA_DATABASE_URL=${DATABASE_URL}
ExecStart=/opt/ryza-a13-fail.sh
FAILUNIT

cat > /etc/systemd/system/ryza-a13.service <<'UNIT'
[Unit]
Description=Ryza A-13 監査(規則⇔実装トレーサビリティ・週次)
After=network-online.target postgresql.service
OnFailure=ryza-a13-fail.service

[Service]
Type=oneshot
Environment=RYZA_DATABASE_URL=${DATABASE_URL}
Environment=GCP_PROJECT=${PROJECT}
ExecStart=/opt/ryza-a13-run.sh
UNIT

cat > /etc/systemd/system/ryza-a13.timer <<'TIMER'
[Unit]
Description=Ryza A-13 監査タイマー(週次)

[Timer]
OnCalendar=${A13_ONCALENDAR}
Persistent=true
...
systemctl enable --now ryza-a13.timer
systemctl list-timers ryza-a13 --no-pager
PROVISION

echo "== 完了 =="
echo "手動実行:   gcloud compute ssh ${VM} --zone ${ZONE} --command 'sudo systemctl start ryza-a13.service'"
echo "ログ確認:   gcloud compute ssh ${VM} --zone ${ZONE} --command 'sudo journalctl -u ryza-a13 -n 30 --no-pager'"
