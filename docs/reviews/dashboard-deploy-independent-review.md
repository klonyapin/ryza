...
- 判定: **条件付き承認**(C-1〜C-3 の是正をもって承認。C-4〜C-7 も同 PR 内で是正)
- 対象: 「次回 PR 対応」3件の是正(ops/deploy-dashboard.sh・ops/lib/deploy-guards.sh・
  ops/lib/pg_hba_check.sh・tests/ops/test_deploy_guards.py・tests/ops/test_deploy_role_gate.py)
- 手法: 前回までの「実装を読む」審査に加え、**変異試験**を行った。統制を無効化する
  改変を実際に当て、追加されたテストがそれを FAIL として捉えるかを見る。テストの
  存在ではなくテストの検出力を評価対象にした(議論規約4)。

### 是正の確認

「次回 PR 対応」3件はいずれも実装されている。ロール検証は `DO $ryza_gate$` ブロックで
例外を上げる形になり、pg_hba 検査はアドレス列のみを見るパーサへ置き換わり、統制は
関数へ切り出されて CI から中断挙動を検証できるようになった。とくに、ロール権限境界を
実 PostgreSQL 上の一時ロールに対して実測する形にした点(tests/ops/test_deploy_role_gate.py)は、
統制の主張を意見でなく証拠で裏づけており評価する。

### マージ阻止(3件)

- **C-1 ゲートの実効が配線に依存し、その配線が無検証**: `RAISE EXCEPTION` が実際に
  デプロイを止めるかは、psql の `-v ON_ERROR_STOP=1`・パイプラインに握り潰しが無いこと・
  リモートヒアドキュメントの `set -euo pipefail` の3点に依存する。変異 M2(ON_ERROR_STOP
  削除)・M3(`|| true` 付加)を当てても全テストが通過した。ゲート本体が正しくても
  配線1文字で無効化でき、それを誰も検出しない。
- **C-2 pg_hba 検査の「検査自体の失敗」が黙認される**: 呼び出しが `|| true` 付きの
  2値判定(出力の有無)で、awk の異常終了やライブラリの読み込み失敗が「想定外なし」と
  同じ経路に落ちる。変異 M4(`eval` 後の関数が未定義になる状況)・M5(検査失敗を 0 に
  倒す)を検出できない。再審査 条件1 で指摘した「取得失敗を公開なしと混同するな」と
  同型の欠陥が、別の検査に残っている。
- **C-3 ロールゲートが列レベル GRANT を見ていない**: 全項目が
  `information_schema.role_table_grants` のみを参照するが、この view には
  `GRANT UPDATE (state) ON ops.trading_state TO ryza_boardroom` のような列指定の
  GRANT が**1行も現れない**。Kill Switch 状態を書き換えられる権限がゲートにも証跡にも
  映らず素通りする。`information_schema.column_privileges` の併用が要る(同 view は
  表レベル GRANT も列へ展開するため、表数を数える検査はその点に注意)。

### 同 PR 内で是正すべき事項(4件)

- **C-4** ゲート 4.5 の期待値を「実在する表の数」から算出しているため、0020 未適用の
  DB(実在 0・GRANT 0)が「一致」と判定される。役員室が何も書けないダッシュボードを
  正常として本番化しうる。`expected_ops_tables <> 2` を例外に。
- **C-5** `EXPECTED_ORIGIN` が env で上書き可能。攻撃者は origin を自分のリモートに
  差し替え、同じ値を env で渡すだけでゲートを満たせる。origin 照合が統制として成立して
  いない。ハードコードした許可リスト(https の .git 有無・SSH)に固定すること。
- **C-6** `samehost` をループバック扱いにしている。samehost はサーバ自身の全 IP
  アドレスに一致し、VM の VPC 内部 IP を含むためループバック限定ではない。
- **C-7** 追記の要否判定が `grep -qF` の完全一致で、検査側の正規化と不一致。空白の
  詰め方が違う既存行があると同義の行を重複追記する(冪等性)。

### 確認済み(懸念なしと判断した項目)

- **例外の伝播**: 生成 SQL の `DO` ブロックを実 PostgreSQL 17 に対して実行し、
  `RAISE EXCEPTION` が psql の終了コード 3 として観測されることを確認した。
  `ON_ERROR_STOP=1` と組み合わされる限り、リモートシェルは確実に中断する
  (依存する配線の無検証は C-1 として別に挙げた)。
- **pg_hba パーサの網羅性**: コメント内 localhost・データベース名 localhost・
  コメントアウト行・netmask 2列表記・samenet・hostssl/hostnossl/hostgssenc/hostnogssenc・
  include 指令・同一 CIDR の広い行など13通りの入力で、いずれも fail-closed 側
  (疑わしきは中断)に倒れることを確認した。反対すべき点を探して見つからなかった。
- **base64 経由の関数転送**: ヒアドキュメントの二重展開を避ける手段として妥当。
  転送内容はリポジトリ内のファイルであり、git ゲートを通過した時点で承認済み main と
  一致することが保証される。新たな信頼境界は増えていない。

### 設計リード裁定(2026-08-03 追記)

- C-1〜C-7 は本 PR 内で是正する(次回 PR への繰延はしない)。統制の配線・検査失敗の
  扱い・列レベル権限はいずれも「壊れていても静かに通る」性質の欠陥であり、
  Phase 5 の Cloud Build トリガー化を待たせる理由がないため。
