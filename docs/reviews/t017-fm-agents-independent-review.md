...
## FM 運用整備 審査記録(2026-08-04 追記)— 0028(保護領域 schema)・検疫 CLI・curated ユニバース

- 対象: `origin/main..HEAD` 3 commits(`migrations/0028_classification_asset_class.sql`・`risk/classify.py`(curated 供給)・`fm/theses.py`(検疫 CLI)・`config/universe/jim-curated.yaml`・`docs/ops/fm-{quarantine-runbook,curated-universe}.md`)。検証: 実 DB(`ryza_rev28`)へ全 migration 適用の上 `pytest tests/risk/test_classify.py tests/test_migrations.py tests/fm` **162 passed**(審査者実行)+ 審査者作成の敵対的プローブ 7 本を実測。判定は**条件付き承認** — DISABLE TRIGGER の前例としての設計は概ね妥当だが、解除の粒度と再構成の痕跡に穴がある。
- **C-22(中・マージ前推奨 — DISABLE TRIGGER の前例設計)**: 0028 は `DISABLE TRIGGER USER` で**当該表の全ユーザトリガ 3 本**(no_mutation・no_truncate・stamp_recorded_at)を落とす。実際に必要なのは `no_mutation` 1 本のみであり、解除範囲が「SET は派生列だけ」という主張どおりかを SQL の字面から検証できない。実測: ランナー経路(`db/migrate.apply_migration` の 1 ファイル=1 トランザクション・`connect()` は `autocommit=False`)では途中失敗後も 3 本とも `tgenabled='O'` に巻き戻る(**原子性は確認済み**)。一方 `psql -f` 相当(文ごと autocommit)で同じ内容を流して失敗させると **3 本とも 'D' のまま残留**する(実測)。デプロイ経路(`ops/deploy-{bot,daily}.sh`)はランナーを使うため現時点の実害は無いが、本 migration は前例として複製されるので、①解除は**名前指定の 1 本**に限る、②末尾に `DO $$ ... IF EXISTS (tgenabled <> 'O') THEN RAISE EXCEPTION` の自己検査を置く、の 2 点を 0026 判断6 が求める「監査つき手続」の最低要件として明文化すべきである。
- **C-23(中・マージ前推奨 — 再構成の痕跡)**: 0026 は再構成行を `backfilled=true` で識別できるようにしたが、0028 は**通常運転で記録された履歴行(`backfilled=false`)の `asset_class` も現行ルールで再構成して書き込む**。実測: `source='rules:v1'`・`backfilled=false` の行が `asset_class='equity_jp'` で埋まり、その値が同時点に記録されたものか 0028 の再構成かを行から区別できない。列コメントに記載はあるが、0026 が「規約ではなくデータで識別する」を選んだ趣旨に反する。是正: `asset_class_backfilled boolean` 列、または `meta` への適用件数記録(「監査つき」の実体)。
- **C-24(中 — curated の承認が統制になっていない)**: `load_curated_universe` の承認検査は `approved_at` / `approved_by` の**非空判定のみ**で、日付形式も承認者の実在も検証しない。実測: `approved_at: いつか` / `approved_by: 誰か` が受理される。さらに `config/universe/**` は `config/governance.yaml` の `protected_areas` に**未登録**であり(`config/mandates/*.yaml` は登録済み)、A-18-1 の `Approved:` トレーラ突合の対象外である。すなわち「銘柄 35 行の追加」と「自分で承認済みと書くこと」を同一 PR・無トレーラで行える。curated タグ付与はマンデートの universe 語彙に属する母集団を実際に埋める操作であり、統制上はマンデート変更と同格である。是正: `config/universe/**` を `protected_areas`(area: mandates)へ登録し、承認の正を YAML 内の文字列ではなく `governance.decisions` / トレーラ側に置く。
- **C-25(低・敵対的視点)**: `rationale` / `criterion` は必須だが**機械検証されない自由文**である。「日経225 構成」と書けば実際の構成銘柄でなくても通り、`source` の `version` もファイル内容ハッシュに紐付かないため、同一 `v1` で内容の異なる 2 版が履歴上区別できない。ローダが止めるのは「根拠欄が空」だけで「根拠が虚偽」ではない。是正の最小形は `source` に内容ハッシュを含めること(監査で適用済みリストを一意に復元できる)。実測基準への移行(reminder `fm-liquid-equity-measured-turnover`)まではこれが唯一の担保になる。
- **C-26(低 — 検疫 CLI)**: `--reason` / `--by` は非空検査のみで、`--by` は「二者」であることを検証しない自由文字列。`--yes` は確認プロンプト(thesis_id 再入力)を丸ごと省くため、runbook が要求する二者確認は**コード上どこにも強制されていない**(runbook §「通常運用では使わない」の宣言のみ)。誤検疫が解除不能である以上、`--yes` は非対話環境限定を環境変数で明示させるか、`--by` に 2 名以上を要求する形式検査を入れるべきである。
- **反対を探して見つからなかった点**: ①**バックフィルの point-in-time は正しい**。審査者が SCD2 で venue が NASDAQ→TSE に変わった銘柄を作り、旧版期間の履歴行に 0028 の UPDATE 文(migration ファイルから実物を抽出)を流したところ `equity_us` が入った(look-ahead なら `equity_jp`)。`i.valid_from <= h.as_of AND (valid_to IS NULL OR valid_to > h.as_of)` は主張どおり機能する。ただし同梱の `test_backfill_derives_asset_class_from_the_instrument_master` は**単一版の銘柄 3 本しか使っておらず**、`valid_to IS NULL` を使う look-ahead 実装でも同じく通る — 判断4 の回帰としては無力なので、上記 SCD2 ケースをテストに加えること(低・推奨)。②語彙の三者一致テストは現在値表・履歴表**両方**の CHECK 制約定義を `pg_get_constraintdef` から実測比較しており(`test_classify.py:395-406`)、片側だけの typo をすり抜けない。③同梱 `jim-curated.yaml` の fail-closed 出荷は実測で確認(ローダが `未承認` で拒否・curated は日次ジョブに配線されておらず手動 CLI のみ)。④付与だけでなく**撤回**を `manages_tags` で config 駆動にした判断は正しく、「config が正」を嘘にしない唯一の形である。⑤`fm/base.ips_asset_class` の削除により分類の正が 1 箇所に集約され、C-4 系の「読出し側が導出する」構造は解消された。
- **この判断が誤っている場合の理由トップ3**: ①C-22 の過大評価(デプロイ経路はランナー固定なので psql 経路は空想)→ ただし前例の複製コストはゼロで、自己検査 3 行の追加コストもゼロであり、争う価値がない。②C-24 の過大評価(curated は手動 CLI でしか叩かれず、実行者=承認者が同一人物の現段階では実質的な差が無い)→ しかし統制は移行期の人数構成ではなく経路の形で評価すべきで、`config/mandates/*.yaml` を保護して `config/universe/**` を保護しない非対称に説明が付かない。③C-23 を過小評価している可能性 — 資産クラスは G-4/G-5 の入力であり、「当時そう分類していた」と「今のルールで再構成した」の区別が付かない履歴は、将来のインシデント調査で E6 の主張範囲を確定できなくする。
