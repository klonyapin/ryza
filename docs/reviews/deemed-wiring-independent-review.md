...
## 審査対象 SHA 突合(0029 / A-18-8)独立役員審査記録(2026-08-04)

対象は `migrations/0029`・`governance/decisions.py` writer/CLI・`audit/a18.check_reviewed_sha_agreement`。判定は **条件付き承認**(SHA-2 を本 PR で是正、SHA-1/3 を本 PR か直後の PR で是正し reminders に登録)。

- **SHA-1(重要)**: 不一致のとき承継を支配するのは依然**トレーラ側**(マージ時に書ける可変値)である。記録側 `reviewed_sha` は追記オンリーで**発効通知の時点に固定**され、48h 異議期間が実際に係属した内容を指す。したがって不一致時は記録側を正として A-18-1 の承継範囲をそこまで縮めるのが定款第3条の意味論に合う。爆風半径論は「承継を全部落とす」案への反論としては妥当だが、**承継を記録側 SHA まで縮める中間案**は検討されていない。現状では発効通知**後**にブランチへ積んだコミットがマージ時トレーラ経由で承認を得る経路が開いたままで、A-18-8 はそれを所見にするだけである。
- **SHA-2(重要・低コスト)**: 非対称の片側が**計測すらされていない**。ループは `declared is None` で `continue` するため「記録側に `reviewed_sha` があるのにトレーラが v1」の件数が `compared` にも `trailer_only` にも入らない。CLI が head SHA を自動格納する以上、今後の記録側はほぼ常に埋まる — 攻撃者でなく**横着**でトレーラから `reviewed=` を落とすだけで、承継は無制限のまま A-18-8 は無音になる。行は既に取得済みなので `record_only` カウンタは数行で足り、`trailer-v1-sunset` の判断材料にもなる。
- **SHA-3(重要)**: A-18-8 の不一致は**恒久的で解消経路が無い**。記録は追記オンリーで訂正不能、main のトレーラは改変不能、`acknowledged_findings` は A-18-1 の violations 専用である。手入力 `--reviewed-sha` の打ち間違い1件で `has_findings` が永久に真になり週次 A-18 が恒常 ⚠️ 化する。空間的な爆風を避けて時間的な爆風を作っており、受容記録(または追記オンリーの訂正表)を A-18-8 にも通すこと。
- **SHA-4(中)**: 3専決(定款・実弾・KS 復帰)が通る明示承認経路 `bot/approvals.py` の INSERT は `reviewed_sha` / `review_ref` を書けず、A-18-8 の射程外である。notes は「0029 以前の記録、または --deemed-for-pr 以外の経路」と**移行的**に読める書き方で、最重要の決定種別が**構造的に**突合対象外であることが伝わらない。
- **SHA-5(中)**: 分母 `compared` はトレーラ**行数**であって決定数ではない(A-18-8 は A-18-1 と違い全コミットの本文を読む)。同一決定を参照するコミットが N 個あれば N 加算され、同じ不一致が N 件の所見として重複する。緑の文言「突合できた決定 N 件」は誤表示であり、`reviewed-sha-mismatch-fail-safe` はこの水増しされた数値を将来判断の材料に指定している。distinct decision_id で数えること。
- **SHA-6(軽微)**: `--review` の実在検査は stderr 警告のみで痕跡が残らない(`meta.runs` の params にも DB にも入らない)ため、事後監査からは「警告が出たか」を判別できない。`missing_review_ref_warning` の `repo_root = parents[3]` はソースチェックアウト前提で、パッケージ設置時は全参照が誤警告になる。A-18-8 は `config/governance.yaml` の統制テーブル(定款第6条)に未登録 — A-18-5/6/7 も同様で、統制表と実装の乖離が累積している。
- **肯定(反対点を探して見つからなかったもの)**: 追記オンリーとの両立主張は正しい(0021 の `decisions_no_mutation` は `BEFORE UPDATE OR DELETE` で後埋めを拒否し、テストが `RaiseException` で固定済み)。view 再定義は 0021 の定義と本文一致で2列を末尾追加したのみ、消費者は全て列名指定のため互換性は保たれる。CHECK 制約は writer を迂回した直 INSERT も止める(テストあり)。`--review` の不在を拒否でなく警告にした判断は遡及登録・リポジトリ外審査を塞がない点で妥当。「一致は証明ではない」という限界の開示が docstring・notes・STANDARD_DISCLOSURES・yaml・reminders に一貫して置かれ、緑表示の文言までテストで固定されている点は評価する。
- **敵対的検証**: 「審査を経ていない SHA を記録とトレーラの両方に書く」自己整合な虚偽は A-18-8 では検出できないが、これは開示済みであり、A-18-1 の第2親祖先検査は不変なので**従来より悪化はしていない**。ただし CLI の head SHA 自動取得により、リマインダー原文が名指しした退化ケース(`reviewed=<マージ直前のブランチ head>` で被覆が v1 と同等)が**既定の挙動**になった点は注意を要する — 統制の強度は不変のまま、監査が肯定的な緑を出すようになったため、表示だけが先行して強化されている。マージ後 PR の `head.sha` は「マージ時点のブランチ先端」を返すので突合は成立するが、squash/rebase マージでは A-18-1 が承継を拒否する一方で A-18-8 は緑になる(緑 = 健全ではない)。`reviewed_sha` がマージの第2親と一致する件数(= 実質的に被覆無制限)を別途開示すべきである。
- 検証: `tests/governance/test_decisions.py` + `tests/audit/test_a18.py` 212 passed、`ruff check` clean(審査者環境で実行)。
