...
| **CIO ペルソナ** | Fable 級(執行側) | 運用戦略の統括、四半期 IPS 批判レポートの起草、代表との対話窓口、研究本部の統括 | 執行役員(CIO) |
...
| **監査部門**(既設) | 別実行環境・read-only | 証跡ベースの事後検証(A-1〜A-10)。意見ではなく証拠で語る | 内部監査+監査役(三様監査の簡易形) |
...
| 会議体 | 頻度 | 出席 | 内容 |
|---|---|---|---|
| **投資委員会(定例)** | 四半期 | 代表+CIO+独立役員 | CIO の IPS 批判レポート → 独立役員の意見書 → 対話 → 決議 |
...
- 役職を選んでチャット(CIO/独立役員)。裏側は「役職資産+RAG+当該役職のモデル」で応答する LLM チャット
- 会話は自動で議事録候補として保存。「決議」ボタンで正式決定に昇格(代表のみ押せる)
- Discord の承認フロー(日常の HITL)とは役割分担: **Discord=日常の承認・通知、役員室=経営レベルの対話・審議**
- 実装コスト: チャット時のみモデルを呼ぶオンデマンド課金。四半期レポートはバッチ。予算科目「役員(Fable)」で管理
