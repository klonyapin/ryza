...
- **会議形式**(2026-08-03 代表指示。従来は役職を選ぶ1対1チャット): 代表が発言すると、**反応すべきと判断された役員だけ**が順に応答する。裏側は「役職資産+RAG+当該役職のモデル」で応答する LLM。1ターンは次の3段で構成する:
  ...
  2. **発言段**(役員の階層): 選ばれた役職だけがルータの与えた順で逐次応答する。各役員には**当該会議のトランスクリプト全文**(代表+先行役員の発言)を渡し、後の発言者は前の発言を踏まえて議論する(§3 の牽制構造 — CIO の提案を独立役員が批判する)。誰も選ばれなかった場合は CIO が進行役として簡潔に応答する(代表の発言を黙殺しない)
  3. **反応ラウンド(最大1回)**: 新たに出た発言に反応すべき役職をルータがもう一度だけ選び、選ばれた役職が応答する
  - **決定論ガード**(不変原則1: 安全側の義務を LLM 判断に依存させない): 代表の発言が重要決定の兆候 — 定数キーワード(定款・実弾・Kill Switch・IPS・マンデート・リスクリミット・コンプラ/ゲート・会計エンジン・スキーマ・監査・資本配分・サイジング・昇格・発注・ポジション・レバレッジ・承認・決議・改訂)または金額・比率の表記(¥/円/%/倍 等) — を含む場合、**ルータの出力に関わらず独立役員を初回ラウンドの発言者に加える**。批判は提案の後に聞くため末尾に置き、枠が埋まっていれば末尾の1名と入れ替える(上限は破らない)。過検出の害(独立役員が1回多く発言する)は未検出の害(重要決定が無批判で通る)より小さいため recall 優先で広く取る
  - **費用の上限はコードで強制する**: 1ターンの役員発言は最大4件で打ち切る(ルータが何人選んでも、ガードが働いても超えない)。ルータの出力をそのまま実行数にしない(不変原則1: LLM は判断材料を作る側)
