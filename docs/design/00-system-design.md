...
### 研究成果の評価設計(2026-08-02 追加)

**核: 事前登録(preregistration)**。仮説・データ範囲・成功基準・テスト計画を、結果を見る前に証憑ストアへ不変登録してからバックテストを実行する(結果を見てから仮説を作る HARKing の構造的防止。オープンサイエンスの標準実務の移植で、リネージ基盤があるため実装コストは小さい)。

| 層 | 指標 | 頻度 |
|---|---|---|
| プロセス(自動・先行) | 執筆規格リンター通過率 / E1〜E9 完全準拠率 / 再現コード付き率 / **事前登録遵守率** | 週次 |
| ファネル(結果・中間) | 仮説→検証→孵化→昇格の各段通過数 / 負の結果数(重複仮説の再検証を防いだ回数も計上) | 月次 |
| 実現(結果・遅行) | 昇格戦略の**前向き実現率**(ペーパーでのライブ成績 ÷ バックテスト予測 — 過剰適合の最終検出器) / 後続研究からの参照数 | 四半期 |
| 品質(盲検) | 独立系統 AI が著者・結論を伏せた本文サンプルを規格・論証品質で採点 | 四半期 |

**アンチゲーミング**: 数値目標は上表の「プロセス層」のみに置き、ファネル・実現層は目標化しない(Goodhart の法則 — 昇格数を目標にすると水増し昇格が起きる)。評価の運用は経営会議、設計改訂は年次のガバナンス実効性評価で。
