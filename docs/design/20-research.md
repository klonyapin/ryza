...
**第2期ソース(正式登録・2026-08-03)**: SEC EDGAR(米企業開示・13F)/ e-Stat(日本政府統計 API)/ ECB・BOE 等の海外中銀 API / 機関投資家フロー(保険・年金の運用計画、投信フロー)/ Reddit・5ch 等コミュニティ。**ソース拡張は継続プロセス**とする — 情報分析班が新ソースを評価し、経営管理部のミニ投資案(期待価値・検証方法・撤退基準)として起案 → 承認後にアダプタ追加。初期スコープを絞るのは、鮮度 SLA・品質監査(A-14)の管理品質を先に固めるため |
