...
| FRED API | 米・グローバルのマクロ統計系列(金利・物価・雇用等) | 日次 | 2026-08-03 追加(ユーザー指摘)。無料 API キー |
| 官公庁・中銀 RSS | 日銀・財務省・金融庁・FRB の公式発表 | 15分間隔 | news_rss の初期セットに含める(確定) |

**一括拡張バッチ(T-012・2026-08-03 改訂)**: 「無料・API/RSS で安定・既存パラダイムで処理可能」なソースは段階化せず、T-009 の共通基盤完成直後に**一括追加**する — SEC EDGAR(米企業開示・13F)/ e-Stat(日本政府統計)/ ECB・BOE・IMF 等の海外中銀・国際機関 API / 主要国の統計局 RSS。当初の「少しずつ拡張」方針はユーザー指摘で撤回(SLA・監査は自動化済みでソース数の限界コストが低く、階層0トリアージが LLM コストを重要度で絞るため。段階化の根拠は AI 労働系では成立しない)

**真に後回しにするもの(パラダイム別・数の問題ではない)**: ①コミュニティ系(Reddit・5ch)— スパム除去・話者信頼性の前処理設計が本体 ②機関投資家フロー(非定型 PDF・スクレイピング)— 抽出パイプラインの設計が本体。どちらも情報分析班の設計課題として起票し、処理設計とセットで導入 |
