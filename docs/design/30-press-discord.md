...
- **日報**: 会計・リスクの確定値を 18:00 JST に `#日報` へ(データが無い間は稼働状況のみ)
- **失敗時**: Bot 死活は Cloud Monitoring の uptime check(プロセスメトリクス)+ Scheduler の朝刊ジョブ側からの疎通確認で検知し、`#経営` へ(Bot 死亡時はメール fallback)
...
1. マスコットの見た目の希望(生成時のスタイル指定。なければ私が数案生成して Discord で選んでもらう)
2. チャンネル構成の確認(§1 の7チャンネル案で良いか、既存サーバーの構成に合わせるか)
